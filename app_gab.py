...
if selected2 == "KPIs":
    ...
    if opcion == "Distribucion de marcas y modelos":
        ...
        @st.cache_data
        def compute_p1():
          fig = px.pie(top_5_values, values='count', names='name', color_discrete_sequence=px.colors.sequential.RdBu)
          fig.update_layout({
              'plot_bgcolor': 'rgba(0, 0, 0, 0)',
              'paper_bgcolor': 'rgba(0, 0, 0, 0)',
              'margin': dict(l=0, r=0, b=0, t=0, pad=0),
    ...
    if opcion == "Distribución de condados":
        @st.cache_data
        def compute_p3():
          fig = px.pie(top_5, values='count', names='name', color_discrete_sequence=px.colors.sequential.Burgyl)
          fig.update_layout({
          'plot_bgcolor': 'rgba(0, 0, 0, 0)',
          'paper_bgcolor': 'rgba(0, 0, 0, 0)',
          'margin': dict(l=0, r=0, b=0, t=0, pad=0),
